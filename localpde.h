...
public:
  ...
  LocalPDE(ParameterReader &param_reader) : state_block_component_(4, 0), control_block_component_(1, 0)
  {
    ...
    for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
    {
      ...
      for (unsigned int i = 0; i < n_dofs_per_element; i++)
      {
        ...
        if (fabs(state_fe_values[multiplier].value(i, q_point) - 1.) < std::numeric_limits<double>::epsilon())
        {
          ...
          Point<2> QPoint = state_fe_values.quadrature_point(q_point);
	  bool AtBoundary = ((QPoint(0) == 1) || (QPoint(0) == 0) || (QPoint(1) == 0) || (QPoint(1) == 1) || ((QPoint(1) == 0.5) && (QPoint(0) >= 0.5))); 
	  //AtBoundary = false;
	  if((n_neig == 4) || AtBoundary)
          {
  ...
  for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
  {
    ...
    for (unsigned int i = 0; i < n_dofs_per_element; i++)
    {
      ...
      local_vector(i) += scale * (
                                     // Main terms
                                     (1 - constant_k_) * (scalar_product(stress_term_plus_LinU, E) + scalar_product(stress_term_plus, E_LinU)) * pf * zPf // du
    ...
    double /*scale_ico*/)
{
  ...
  for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
  {
    ...
    Tensor<1, 2> dzDisp; //TODO: This and other Disp in different fncts can probably be deleted
	...
	if((n_neig == 4) || AtBoundary)
        {
          ...
          if ((uvalues_[q_point][3] + s_ * (pf - old_timestep_pf)) <= 0.)
          {
            local_vector(i) += scale * weight * dzMult;
          }
          else // max > 0
          {
            // From Complementarity
            for (unsigned int j = 0; j < n_dofs_per_element; j++)
            {
              if (fabs(state_fe_values[phasefield].value(j, q_point) - 1.) < std::numeric_limits<double>::epsilon())
              {
                local_vector(j) -= scale * weight * s_ * dzMult;
              }
            }
          }
          // From Equation
          local_vector(i) += scale * weight * dzPf;
        }
        else // Boundary or hanging node no weight so it works when hanging
        {
...
GetUpdateFlags() const
{
  if (this->problem_type_ == "state" || this->problem_type_ == "adjoint" || this->problem_type_ == "adjoint_hessian" || this->problem_type_ == "tangent")
    ...
    return update_values | update_quadrature_points | update_gradients; //added gradients here, not sure if ok TODO
  else
